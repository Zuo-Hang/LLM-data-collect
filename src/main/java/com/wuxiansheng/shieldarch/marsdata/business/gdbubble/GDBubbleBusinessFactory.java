...
@Slf4j
@Component
public class GDBubbleBusinessFactory implements BusinessFactory {
    ...
    public Business createBusiness(String msg) {
        try {
            ...
            GDBubbleInput input = new GDBubbleInput();
            input.setEstimateId(getStringValue(parser, "objId"));
            input.setActivityName(getStringValue(parser, "activityName"));
            
            // 使用QuestImageUrls（不是V2版本）
            String imageData = getStringValue(parser, "data.data397");
            input.setBubbleImageUrls(questUtils.questImageUrls(imageData));
            ...
            input.setClientTime(clientTime);
            ...
            input.setRidePlatformId(2);
            input.setRidePlatformName("高德");
            
            GDBubbleBusiness business = new GDBubbleBusiness();
            business.setInput(input);
            
            return business;
            
        } catch (Exception e) {
            log.error("创建高德冒泡业务失败: msg={}, error={}", msg, e.getMessage(), e);
